...
// ------Back
    ...
    public function membreBlogUpdate(Article $article, Request $request)
    {
        ...
        // return $this->render('blog/membre/update.html.twig', [
        //     'form' => $form-> createView(),
        //     'article' => $article,
    ...
    public function handleForm(Article $article, Request $request, bool $new){
        ...
        if($image){
            ...
            $this-> addFlash('success' , "L'article a bien été " . ($new ? 'créé' : 'modifier'));
            return $this->redirectToRoute('membre_blog_update', ['id'=> $article->getId()]);
    ...
    {
        $this->denyAccessUnlessGranted(ArticleVoter::DELETE, $article ,"Arretez de supprimer des articles des autres!!!!");
        $em = $this->getDoctrine()->getManager();
        $em->remove($article);
        $em->flush();
